...
//      std::uint32_t x_mask = (1 << ((~(param->src_bits>>1)) & 3)) - 1;
  ...
  void LGFXBase::push_image_affine_aa(const float* matrix, pixelcopy_t* pc, pixelcopy_t* pc2)
  {
    ...
    pc->src_x32_add = iA[0];
    pc->src_y32_add = iA[3];
    std::uint32_t x32_diff = std::min<std::int32_t>(8 << FP_SCALE, std::max(abs(iA[0]), abs(iA[1])) - 1) >> 1;
    std::uint32_t y32_diff = std::min<std::int32_t>(8 << FP_SCALE, std::max(abs(iA[3]), abs(iA[4])) - 1) >> 1;
