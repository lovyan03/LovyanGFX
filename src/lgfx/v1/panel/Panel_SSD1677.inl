...
//----------------------------------------------------------------------------
  ...
  // endpoints.
  static constexpr uint8_t lut_quality_SSD1677[110] = {
  ...
  static constexpr uint8_t lut_text_SSD1677[110] = {
  ...
  static constexpr uint8_t lut_fast_SSD1677[110] = {
    0x55, 0x55, 0x55, 0x55, 0x55, 0x5A, 0xAA, 0xAA, 0x00, 0x00, // white
  ...
  static constexpr uint8_t lut_fastest_SSD1677[110] = {
      ...
      const range_rect_t& current_dirty)
  {
    if (!_displayed_valid)
    {
      ...
      return _refresh_mode2_absolute(lsb, msb, lut_fast_SSD1677);
    }
    if (!_ensure_known_face()) { return false; }

    _send_transition_planes(msb, current_dirty);
    _send_gray_lut(lut_fastest_SSD1677);
    if (!_activate(CTRL1_NORMAL, 0x0C, false, true, false)) { return false; }
  ...
  void Panel_SSD1677_4Gray::display(uint_fast16_t x, uint_fast16_t y, uint_fast16_t w, uint_fast16_t h)
  {
    ...
    case epd_mode_t::epd_quality:
      success = _refresh_mode1_absolute(planeL, planeM, lut_quality_SSD1677);
      break;
    case epd_mode_t::epd_text:
      success = _refresh_mode1_absolute(planeL, planeM, lut_text_SSD1677);
      break;
    case epd_mode_t::epd_fast:
      success = _refresh_mode2_absolute(planeL, planeM, lut_fast_SSD1677);
      break;
