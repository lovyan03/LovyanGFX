...
//----------------------------------------------------------------------------
  ...
  void Panel_GDEW0154M09::setSleep(bool flg_sleep)
  {
    if (flg_sleep)
    {
      startWrite();
      _bus->writeCommand(0x50, 8); // VCOM and DATA
  ...
  void Panel_GDEW0154M09::setIdol(bool flg)
  {
    startWrite();
    _bus->writeCommand(0x50, 8);
    if (flg)
