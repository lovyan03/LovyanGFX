...
#error "Panel_GDEW0154D67.inl is part of lgfx_v1.cpp and is not meant to be included on its own"
#endif
...
//----------------------------------------------------------------------------
  ...
  // File-local command codes (lower case: Panel_SSD1677 has the same names in upper case).
  static constexpr uint8_t cmd_deep_sleep_mode              = 0x10;
  ...
  static constexpr uint8_t Bayer_GDEW0154D67[16] = { 8, 200, 40, 232, 72, 136, 104, 168, 56, 248, 24, 216, 120, 184, 88, 152 };
...
*/
  ...
  void Panel_GDEW0154D67::writeFillRectPreclipped(uint_fast16_t x, uint_fast16_t y, uint_fast16_t w, uint_fast16_t h, uint32_t rawcolor)
  {
    ...
    do
    {
      ...
      auto btbl = &Bayer_GDEW0154D67[(y & 3) << 2];
      do
      {
...
// printf("time:%d\n", millis() - start_time);
  ...
  void Panel_GDEW0154D67::_draw_pixel(uint_fast16_t x, uint_fast16_t y, uint32_t value)
  {
    _rotate_pos(x, y);
    uint32_t idx = ((_cfg.panel_width + 7) & ~7) * y + x;
    bool flg = 256 <= value + Bayer_GDEW0154D67[(x & 3) | (y & 3) << 2];
    if (flg) _buf[idx >> 3] |=   0x80 >> (idx & 7);
    else     _buf[idx >> 3] &= ~(0x80 >> (idx & 7));
  }
