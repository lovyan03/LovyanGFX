...
bool Bus_Parallel16::writeCommand(uint32_t data, uint_fast8_t bit_length)
{
    ...
    *reg_lcd_user =
        LCD_CAM_LCD_CMD_2_CYCLE_EN | LCD_CAM_LCD_2BYTE_EN | LCD_CAM_LCD_CMD | LCD_CAM_LCD_UPDATE | LCD_CAM_LCD_START;
    return true;
}
...
void Bus_Parallel16::writeData(uint32_t data, uint_fast8_t bit_length)
{
    ...
    if (bytes > 1)
    {
        ...
        if (bytes == 4)
        {
            ...
            *reg_lcd_user = LCD_CAM_LCD_CMD_2_CYCLE_EN | LCD_CAM_LCD_2BYTE_EN | LCD_CAM_LCD_CMD | LCD_CAM_LCD_UPDATE |
                            LCD_CAM_LCD_START;
            return;
        }
        while (*reg_lcd_user & LCD_CAM_LCD_START)
...
void Bus_Parallel16::writeBytes(const uint8_t *data, uint32_t length, bool dc, bool use_dma)
{
    ...
    if ((length + _has_align_data) > 1)
    {
        do
        {
            ...
            {
                size_t len =
                    (length > CACHE_SIZE) ? (((length - 1) % (CACHE_SIZE - _has_align_data)) + 4) & ~3u : length;
            ...
            dev->lcd_misc.val = dc ? LCD_CAM_LCD_CD_IDLE_EDGE
            ...
            *reg_lcd_user = LCD_CAM_LCD_ALWAYS_OUT_EN | LCD_CAM_LCD_2BYTE_EN | LCD_CAM_LCD_CMD_2_CYCLE_EN |
                            LCD_CAM_LCD_DOUT | LCD_CAM_LCD_CMD | LCD_CAM_LCD_UPDATE | LCD_CAM_LCD_START;
        } while (length & ~1u);
    }
    if (length)
    {
...
void Bus_Parallel16::_read_bytes(uint8_t *dst, uint32_t length)
{
    ...
    uint32_t mh = ((((((((((((((((_cfg.pin_d8 & 7)) << 3) + (_cfg.pin_d9 & 7)) << 3) + (_cfg.pin_d10 & 7)) << 3) +
                           (_cfg.pin_d11 & 7))
                          << 3) +
                         (_cfg.pin_d12 & 7))
                        << 3) +
    ...
    uint32_t ih = ((((((((((((((((_cfg.pin_d8 >> 3)) << 3) + (_cfg.pin_d9 >> 3)) << 3) + (_cfg.pin_d10 >> 3)) << 3) +
                           (_cfg.pin_d11 >> 3))
                          << 3) +
                         (_cfg.pin_d12 >> 3))
                        << 3) +
    ...
    uint32_t ml = ((((((((((((((((_cfg.pin_d0 & 7)) << 3) + (_cfg.pin_d1 & 7)) << 3) + (_cfg.pin_d2 & 7)) << 3) +
                           (_cfg.pin_d3 & 7))
                          << 3) +
                         (_cfg.pin_d4 & 7))
                        << 3) +
    ...
    uint32_t il = ((((((((((((((((_cfg.pin_d0 >> 3)) << 3) + (_cfg.pin_d1 >> 3)) << 3) + (_cfg.pin_d2 >> 3)) << 3) +
                           (_cfg.pin_d3 >> 3))
                          << 3) +
                         (_cfg.pin_d4 >> 3))
                        << 3) +
