...
#error "Bus_RGB.inl is part of lgfx_v1.cpp and is not meant to be included on its own"
#endif
...
//----------------------------------------------------------------------------
  ...
  static lcd_cam_dev_t* getDev_rgb(int port)
  {
    return &LCD_CAM;
  }
  ...
  IRAM_ATTR void Bus_RGB::lcd_default_isr_handler(void *args)
  {
    ...
    auto dev = getDev_rgb(me->config().port);
