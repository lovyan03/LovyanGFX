...
void calcClockDiv(uint32_t *div_a, uint32_t *div_b, uint32_t *div_n, uint32_t *clkcnt, uint32_t baseClock,
                  uint32_t targetFreq)
{
...
void pinMode(int_fast16_t pin, pin_mode_t mode)
{
    ...
    if ((size_t)gpio_num >= GPIO_NUM_MAX)
        return;
    ...
    *gpio_en_reg = 1u << (pin & 31);
...
#endif
    {
        ...
        case pin_mode_t::input_pullup:
            io_mux_val |= FUN_PU_M | SLP_PU_M;
            break;
        case pin_mode_t::input_pulldown:
            io_mux_val |= FUN_PD_M | SLP_PD_M;
            break;
        default:
            break;
        }
...
void pin_backup_t::backup(void)
{
    ...
    if (pin_num < GPIO_NUM_MAX)
    {
        ...
        _gpio_enable = *reinterpret_cast<uint32_t *>(((_pin_num & 32) ? GPIO_ENABLE1_REG : GPIO_ENABLE_REG)) &
                       (1 << (_pin_num & 31));
#else
...
void pin_backup_t::restore(void)
{
    ...
    if (pin_num < GPIO_NUM_MAX)
    {
        ...
        // ESP_LOGD("DEBUG","restore IO_MUX_GPIO0_REG          :%08x -> %08x ",
        // *reinterpret_cast<uint32_t*>(GPIO_PIN_MUX_REG[_pin_num]                 ), _io_mux_gpio_reg   );
        // ESP_LOGD("DEBUG","restore GPIO_PIN0_REG             :%08x -> %08x ",
        // *reinterpret_cast<uint32_t*>(GPIO_PIN0_REG              + (_pin_num * 4)), _gpio_pin_reg      );
        // ESP_LOGD("DEBUG","restore GPIO_FUNC0_OUT_SEL_CFG_REG:%08x -> %08x ",
...
bool command(command_t cmd, uint8_t val)
{
    ...
    case command_read:
        res = gpio_in(val);
        break;
    case command_write_low:
        gpio_lo(val);
        break;
    case command_write_high:
        gpio_hi(val);
        break;
    case command_delay:
        delay(val);
    ...
    default:
        if ((cmd >> 2) == (command_mode_output >> 2))
        {
            ...
            switch (cmd)
            {
            case command_mode_input:
                mode = pin_mode_t::input;
                break;
            case command_mode_input_pulldown:
                mode = pin_mode_t::input_pulldown;
                break;
            case command_mode_input_pullup:
                mode = pin_mode_t::input_pullup;
                break;
            default:
                break;
            }
...
{
    // ESP_LOGI("LGFX","spi::init host:%d, sclk:%d, miso:%d, mosi:%d, dma:%d", spi_host, spi_sclk, spi_miso, spi_mosi,
    // dma_channel);
    ...
    if (spi_sclk >= 0)
    {
        gpio_lo(spi_sclk); // ここでLOWにしておくことで、pinMode変更によるHIGHパルスが出力されるのを防止する
                           // (CSなしパネル対策);
    }
    ...
    *reg(SPI_USER_REG(spi_port)) =
        SPI_USR_MOSI | SPI_USR_MISO | SPI_DOUTDIN; // need SD card access (full duplex setting)
...
static void i2c_set_cmd(i2c_dev_t *dev, uint8_t index, uint8_t op_code, uint8_t byte_num, bool flg_nack = false)
{
    ...
    */
    uint32_t cmd_val =
        byte_num |
        ((op_code == i2c_cmd_write || op_code == i2c_cmd_stop) ? 0x100 : 0) // writeおよびstop時はACK_ENを有効にする;
        | op_code << 11;
...
#endif
    i2c_set_pin((i2c_port_t)i2c_port, sda_io, scl_io, gpio_pullup_t::GPIO_PULLUP_ENABLE,
...
static cpp::result<void, error_t> i2c_wait(int i2c_port, bool flg_stop = false)
{
    ...
    if (i2c_context[i2c_port].state == i2c_context_t::state_disconnect)
    {
        return res;
    }
    auto dev = getDev(i2c_port);
    ...
    static constexpr uint32_t intmask =
        I2C_ACK_ERR_INT_RAW_M | I2C_END_DETECT_INT_RAW_M | I2C_ARBITRATION_LOST_INT_RAW_M;
...
#if defined(CONFIG_IDF_TARGET_ESP32C3)
            uint32_t us_limit =
                (dev->scl_high_period.period + dev->scl_low_period.period + 16) * (1 + dev->sr.tx_fifo_cnt);
#elif defined(CONFIG_IDF_TARGET_ESP32S3)
            uint32_t us_limit = (dev->scl_high_period.scl_high_period + dev->scl_low_period.scl_low_period + 16) *
                                (1 + dev->sr.txfifo_cnt);
#else
            uint32_t us_limit =
                (dev->scl_high_period.period + dev->scl_low_period.period + 16) * (1 + dev->status_reg.tx_fifo_cnt);
#endif
    ...
    if (flg_stop || res.has_error())
    {
        ...
        else
        {
            ...
            static constexpr uint32_t intmask_ = I2C_ACK_ERR_INT_RAW_M | I2C_TIME_OUT_INT_RAW_M |
...
{
    if ((i2c_port >= I2C_NUM_MAX) || ((uint32_t)pin_scl >= GPIO_NUM_MAX) || ((uint32_t)pin_sda >= GPIO_NUM_MAX))
    {
        return cpp::fail(error_t::invalid_arg);
    }

    if (i2c_context[i2c_port].initialized && i2c_context[i2c_port].pin_scl == (gpio_num_t)pin_scl &&
...
{
    if ((i2c_port >= I2C_NUM_MAX) || ((uint32_t)i2c_context[i2c_port].pin_scl >= GPIO_NUM_MAX) ||
...
// ESP_LOGI("LGFX", "cpu_freq_conf.source_freq_mhz :%d", cpu_freq_conf.source_freq_mhz);
        ...
        uint32_t scl_high_offset =
            (dev->scl_filter_cfg.en ? (dev->scl_filter_cfg.thres <= 2 ? 8 : (6 + dev->scl_filter_cfg.thres)) : 7);
        ...
        dev->scl_stop_setup.scl_stop_setup_time =
            cycle; // the clock num between the posedge of SCL and the posedge of SDA
        dev->scl_start_hold.scl_start_hold_time =
            cycle; // the clock num between the negedge of SDA and negedge of SCL for start mark
        dev->scl_rstart_setup.scl_rstart_setup_time =
            cycle; // the clock num between the posedge of SCL and the negedge of SDA for restart mark
#else
...
cpp::result<void, error_t> beginTransaction(int i2c_port, int i2c_addr, uint32_t freq, bool read)
{
    ...
    if ((uint32_t)i2c_context[i2c_port].pin_sda >= GPIO_NUM_MAX ||
        (uint32_t)i2c_context[i2c_port].pin_scl >= GPIO_NUM_MAX)
        return cpp::fail(error_t::invalid_arg);

    // ESP_LOGI("LGFX", "i2c::beginTransaction : port:%d / addr:%02x / freq:%d / rw:%d", i2c_port, i2c_addr, freq,
    // read);
    ...
    i2c_set_pin((i2c_port_t)i2c_port, i2c_context[i2c_port].pin_sda, i2c_context[i2c_port].pin_scl,
                gpio_pullup_t::GPIO_PULLUP_ENABLE, gpio_pullup_t::GPIO_PULLUP_ENABLE, I2C_MODE_MASTER);
...
cpp::result<void, error_t> writeBytes(int i2c_port, const uint8_t *data, size_t length)
{
    ...
    if (i2c_context[i2c_port].state != i2c_context_t::state_write)
    {
...
cpp::result<void, error_t> readBytes(int i2c_port, uint8_t *readdata, size_t length, bool last_nack = false)
{
    ...
    if (i2c_context[i2c_port].state != i2c_context_t::state_read)
    {
        return cpp::fail(error_t::mode_mismatch);
    }
    cpp::result<void, error_t> res{};
    ...
    static constexpr uint32_t intmask =
        I2C_ACK_ERR_INT_RAW_M | I2C_TIME_OUT_INT_RAW_M | I2C_END_DETECT_INT_RAW_M | I2C_ARBITRATION_LOST_INT_RAW_M;
    auto fifo_addr = getFifoAddr(i2c_port);
    auto dev = getDev(i2c_port);

    size_t len = 0;
#if defined(CONFIG_IDF_TARGET_ESP32S3)
    uint32_t us_limit = ((dev->scl_high_period.scl_high_period + dev->scl_high_period.scl_wait_high_period +
                          dev->scl_low_period.scl_low_period)
                         << 1);
    ...
    do
    {
        ...
        do
        {
            ...
            while (0 == getRxFifoCount(dev) && !(dev->int_raw.val & intmask) &&
                   ((lgfx::micros() - us) <= us_limit + 1024))
            {
                taskYIELD();
            }
...
cpp::result<void, error_t> transactionWrite(int i2c_port, int addr, const uint8_t *writedata, uint8_t writelen,
                                            uint32_t freq)
{
    cpp::result<void, error_t> res;
...
cpp::result<void, error_t> transactionWriteRead(int i2c_port, int addr, const uint8_t *writedata, uint8_t writelen,
                                                uint8_t *readdata, size_t readlen, uint32_t freq)
{
    cpp::result<void, error_t> res;
    if ((res = beginTransaction(i2c_port, addr, freq, false)).has_value() &&
        (res = writeBytes(i2c_port, writedata, writelen)).has_value() &&
...
cpp::result<void, error_t> writeRegister8(int i2c_port, int addr, uint8_t reg, uint8_t data, uint8_t mask,
                                          uint32_t freq)
{
