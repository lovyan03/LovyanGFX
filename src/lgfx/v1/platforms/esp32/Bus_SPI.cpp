...
static void gpio_reset(size_t pin)
{
    ...
    // gpio_matrix_in には、ArduinoESP32
    // v1.0.x系では重大なバグがある。(無関係なピンに対して設定変更が行われることがある) gpio_matrix_in( (gpio_num_t)pin,
    // 0x100, 0   );
}
...
/// ESP32-C3 で HIGHPART を使用すると異常動作するため分岐する;
    ...
    if (highpart)
        *_spi_user_reg = user_reg | SPI_USR_MOSI_HIGHPART;
    exec_spi();
    if (0 == (length -= len))
        return;
...
void Bus_SPI::addDMAQueue(const uint8_t *data, uint32_t length)
{
    ...
    if (_dma_queue_capacity < new_size)
    {
        ...
        if (_dma_queue != nullptr)
        {
            heap_free(_dma_queue);
        }
...
/// ESP32-C3 で HIGHPART を使用すると異常動作するため分岐する;
        ...
        do
        {
            ...
            else
            {
                ...
                if (highpart)
                    user = userreg | SPI_USR_MISO_HIGHPART;
                if (length < len1)
                {
                    len1 = length;
                    wait_spi();
