...
//----------------------------------------------------------------------------
  ...
  void Bus_Parallel::config(const config_t& config)
  {
    _cfg = config;
  }
  ...
  void Bus_Parallel::init(void)
  {
    ...
    *reg(I2S_INT_ENA_REG(_cfg.i2s_port)) |= I2S_TX_REMPTY_INT_ENA;
  ...
  void Bus_Parallel::_init_pin(void)
  {
  ...
  void Bus_Parallel::_alloc_dmadesc(size_t len)
  {
  ...
  void Bus_Parallel::release(void)
  {

  }

  void Bus_Parallel::beginTransaction(void)
  {
    std::uint32_t freq_apb = getApbFrequency();
    if (_last_freq_apb != freq_apb || _last_freq_write != _cfg.freq_write)
    {
      _last_freq_apb = freq_apb;
      _last_freq_write = _cfg.freq_write;
  ...
  void Bus_Parallel::endTransaction(void)
  {
    wait_i2s();
  }

  void Bus_Parallel::wait_i2s(void)
  {
    ...
    *reg(I2S_CONF_REG(_cfg.i2s_port)) = conf_reg1;
  }

  void Bus_Parallel::wait(void)
  {
    auto conf_reg1 = _conf_reg_default | I2S_TX_RESET | I2S_RX_RESET | I2S_RX_FIFO_RESET;
  ...
  bool Bus_Parallel::busy(void) const
  {
    return (!(*reg(I2S_STATE_REG(_cfg.i2s_port)) & I2S_TX_IDLE));
  }

  void Bus_Parallel::writeCommand(std::uint32_t data, std::uint_fast8_t bit_length)
  {
    wait();
    *reg(I2S_SAMPLE_RATE_CONF_REG(_cfg.i2s_port)) = _sample_rate_conf_reg_32bit;
    *reg(I2S_FIFO_CONF_REG(_cfg.i2s_port)) = _fifo_conf_default;
    *reg(SAFE_I2S_FIFO_WR_REG(_cfg.i2s_port)) = data << 16;
    *reg(I2S_CONF_REG(_cfg.i2s_port)) = _conf_reg_start;
  }

  void Bus_Parallel::writeData(std::uint32_t data, std::uint_fast8_t bit_length)
  {
      wait();
      *reg(I2S_SAMPLE_RATE_CONF_REG(_cfg.i2s_port)) = _sample_rate_conf_reg_32bit;
      *reg(I2S_FIFO_CONF_REG(_cfg.i2s_port)) = _fifo_conf_default;
      *reg(SAFE_I2S_FIFO_WR_REG(_cfg.i2s_port)) = (0x100 | (data & 0xFF)) << 16;
      while (bit_length -= 8) {
        data >>= 8;
        *reg(SAFE_I2S_FIFO_WR_REG(_cfg.i2s_port)) = (0x100 | (data & 0xFF)) << 16;
      }
  ...
  void Bus_Parallel::writeDataRepeat(std::uint32_t color_raw, std::uint_fast8_t bit_length, std::uint32_t length)
  {
    if (bit_length == 16)
    {
      *reg(I2S_SAMPLE_RATE_CONF_REG(_cfg.i2s_port)) = _sample_rate_conf_reg_16bit;
      ...
      do
      {
        ...
        *reg(I2S_FIFO_CONF_REG(_cfg.i2s_port)) = _fifo_conf_default;
        ...
        while (limit--)
        {
          *reg(SAFE_I2S_FIFO_WR_REG(_cfg.i2s_port)) = data;
        }
        *reg(I2S_CONF_REG(_cfg.i2s_port)) = conf_start;
        limit = 32;
      } while (length);
    }
    else
      ...
      *reg(I2S_SAMPLE_RATE_CONF_REG(_cfg.i2s_port)) = _sample_rate_conf_reg_16bit;
      ...
      do
      {
        ...
        *reg(I2S_FIFO_CONF_REG(_cfg.i2s_port)) = _fifo_conf_default;
        while (limit--)
        {
          *reg(SAFE_I2S_FIFO_WR_REG(_cfg.i2s_port)) = data0;
          *reg(SAFE_I2S_FIFO_WR_REG(_cfg.i2s_port)) = data1;
          *reg(SAFE_I2S_FIFO_WR_REG(_cfg.i2s_port)) = data2;
        }
        *reg(I2S_CONF_REG(_cfg.i2s_port)) = conf_start;
        limit = 10;
      } while (length);
  ...
  void Bus_Parallel::writePixels(pixelcopy_t* param, std::uint32_t length)
  {
    std::uint8_t buf[512];
  ...
  void Bus_Parallel::writeBytes(const std::uint8_t* data, std::uint32_t length, bool use_dma)
  {
    auto conf_start = _conf_reg_start;
    static constexpr std::uint32_t data_wr = 0x01000100;
    ...
    *reg(I2S_FIFO_CONF_REG(_cfg.i2s_port)) = _fifo_conf_default;
    ...
    length -= limit << 1;
    do {
      *reg(SAFE_I2S_FIFO_WR_REG(_cfg.i2s_port)) = data[0] << 16 | data[1] | data_wr;
      data += 2;
    } while (--limit);
    *reg(I2S_CONF_REG(_cfg.i2s_port)) = conf_start;
    ...
    do {
      ...
      *(std::uint32_t*)_dmadesc = (((i<<2) + 3) &  ~3 ) | i << 14 | 0xC0000000;
      wait();
      *reg(I2S_FIFO_CONF_REG(_cfg.i2s_port)) = _fifo_conf_dma;
      *reg(I2S_OUT_LINK_REG(_cfg.i2s_port)) = I2S_OUTLINK_START | ((uint32_t)_dmadesc & I2S_OUTLINK_ADDR);
      *reg(I2S_CONF_REG(_cfg.i2s_port)) &= ~(I2S_TX_RESET | I2S_RX_RESET | I2S_TX_START);
      *reg(I2S_CONF_REG(_cfg.i2s_port)) = _conf_reg_default;
      *reg(I2S_INT_CLR_REG(_cfg.i2s_port)) = ~0;
      *reg(I2S_CONF_REG(_cfg.i2s_port)) |= conf_start;
      if (lbase != 256) lbase <<= 1;
    } while (length);
  }

  std::uint_fast8_t Bus_Parallel::_reg_to_value(std::uint32_t raw_value)
  {
  ...
  void Bus_Parallel::beginRead(void)
  {
      wait();
      gpio_lo(_cfg.pin_rd);
...
//*/
  ...
  void Bus_Parallel::endRead(void)
  {
    wait();
    _init_pin();
  }

  std::uint32_t Bus_Parallel::readData(std::uint_fast8_t bit_length)
  {
    union {
      std::uint32_t res;
      std::uint8_t raw[4];
  ...
  void Bus_Parallel::readBytes(std::uint8_t* dst, std::uint32_t length, bool use_dma)
  {
    do {
      std::uint32_t tmp = GPIO.in;   // dummy read speed tweak.
      tmp = GPIO.in;
  ...
  void Bus_Parallel::readPixels(void* dst, pixelcopy_t* param, std::uint32_t length)
  {
