...
#error "Bus_Parallel16.inl is part of lgfx_v1.cpp and is not meant to be included on its own"
#endif
...
//----------------------------------------------------------------------------
  ...
  static constexpr size_t CACHE_THRESH_p16 = 128;

  // static constexpr uint32_t _conf_reg_default_p16 = I2S_TX_MSB_RIGHT | I2S_TX_RIGHT_FIRST | I2S_RX_RIGHT_FIRST | I2S_TX_DMA_EQUAL;
  static constexpr uint32_t _conf_reg_default_p16 = I2S_TX_RIGHT_FIRST | I2S_RX_RIGHT_FIRST | I2S_TX_DMA_EQUAL;
  static constexpr uint32_t _conf_reg_start_p16   = _conf_reg_default_p16 | I2S_TX_START;
  static constexpr uint32_t _conf_reg_reset_p16   = _conf_reg_default_p16 | I2S_TX_RESET;
  ...
  static constexpr uint32_t _fifo_conf_default_p16 = 1 << I2S_TX_FIFO_MOD | 1 << I2S_RX_FIFO_MOD | 32 << I2S_TX_DATA_NUM_S | 32 << I2S_RX_DATA_NUM_S | I2S_TX_FIFO_MOD_FORCE_EN;
  static constexpr uint32_t _fifo_conf_dma_p16     = _fifo_conf_default_p16 | I2S_DSCR_EN;


  static i2s_dev_t* getDev_p16(i2s_port_t port)
  {
#if defined (CONFIG_IDF_TARGET_ESP32S2)
    return &I2S0;
#else
  ...
  bool Bus_Parallel16::init(void)
  {
    ...
    i2s_dev->conf.val = _conf_reg_default_p16;
  ...
  void Bus_Parallel16::beginTransaction(void)
  {
    ...
    i2s_dev->fifo_conf.val = _fifo_conf_dma_p16;
  ...
  size_t Bus_Parallel16::_flush(size_t count, bool dc)
  {
    ...
    *(uint32_t*)&_dmadesc[0] = count << 2 | count << 14 | 0xC0000000;
    ...
    i2s_dev->conf.val = _conf_reg_reset_p16;
    i2s_dev->sample_rate_conf.val = _sample_rate_conf_reg_bufferd;
    ...
    i2s_dev->conf.val = _conf_reg_start_p16;
    if (slow)
    {
      wait = _div_num >> 1;
      do { __asm__ __volatile__ ("nop"); } while (--wait);
    ...
    if (bytes == 2)
    {
      ...
      while (length)
      {
        ...
        *(uint32_t*)(&c[idx]) = color_raw;
        ...
        {
          _flush(idx);
          if (++step == 2 && length >= CACHE_THRESH_p16)
          {
            memcpy(_cache_flip, c, CACHE_THRESH_p16 * sizeof(uint32_t));
            do
            {
              _flush(CACHE_THRESH_p16);
              length -= CACHE_THRESH_p16;
            } while (length >= CACHE_THRESH_p16);
            idx = length;
            break;
          }
          c = _cache_flip;
...
/*
          if (++step == 2)
          {
            if (length > ((CACHE_THRESH_p16 / 3) * 2))
            {
              memcpy(_cache_flip, c, CACHE_THRESH_p16 * sizeof(uint32_t));
              do
              {
                _flush(((CACHE_THRESH_p16 / 3) * 3));
                length -= ((CACHE_THRESH_p16 / 3) * 2);
              } while (length >= ((CACHE_THRESH_p16 / 3) * 2));
              idx = (length >> 1) * 3;
              length = length & 1;
              break;
            }
...
//*/
  ...
  void Bus_Parallel16::writePixels(pixelcopy_t* param, uint32_t length)
  {
    ...
    const uint32_t limit = (CACHE_THRESH_p16 * sizeof(_cache[0][0])) / bytes;
  ...
  void Bus_Parallel16::writeBytes(const uint8_t* data, uint32_t length, bool dc, bool use_dma)
  {
    ...
    if ((length + _has_align_data) > 1)
    {
      do
      {
        ...
        {
          size_t len = ((length - 1) % (CACHE_THRESH_p16 * sizeof(_cache[0][0]))) + 1;
