...
size_t IFont::drawCharDummy(LGFXBase *gfx, int32_t x, int32_t y, int32_t w, int32_t h, const TextStyle *style,
...
size_t GLCDfont::drawChar(LGFXBase *gfx, int32_t x, int32_t y, uint16_t c, const TextStyle *style, FontMetrics *metrics,
                          int32_t &filled_x) const
{
    (void)metrics;
    auto info = reinterpret_cast<const glcd_fontinfo_t *>(widthtbl);
...
static size_t draw_char_bmp(LGFXBase *gfx, int32_t x, int32_t y, const TextStyle *style, const uint8_t *font_addr,
                            int_fast8_t fontWidth, int_fast8_t fontHeight, int_fast8_t w, int_fast8_t margin)
{
    uint32_t colortbl[2] = {gfx->getColorConverter()->convert(style->back_rgb888),
                            gfx->getColorConverter()->convert(style->fore_rgb888)};
    bool fillbg = (style->back_rgb888 != style->fore_rgb888);
...
size_t FixedBMPfont::drawChar(LGFXBase *gfx, int32_t x, int32_t y, uint16_t uniCode, const TextStyle *style,
                              FontMetrics *metrics, int32_t &filled_x) const
{ // BMP font
    (void)metrics;
    const int_fast16_t fontHeight = this->height;

    auto info = reinterpret_cast<const glcd_fontinfo_t *>(widthtbl);
    if (info->start > uniCode || uniCode > info->end)
    {
        return drawCharDummy(gfx, x, y, width, fontHeight, style, filled_x);
    }
...
size_t BMPfont::drawChar(LGFXBase *gfx, int32_t x, int32_t y, uint16_t uniCode, const TextStyle *style,
                         FontMetrics *metrics, int32_t &filled_x) const
{ // BMP font
    (void)metrics;
    if ((uniCode -= 0x20u) >= 0x60u)
        return drawCharDummy(gfx, x, y, this->widthtbl[0], this->height, style, filled_x);
...
size_t BDFfont::drawChar(LGFXBase *gfx, int32_t x, int32_t y, uint16_t c, const TextStyle *style, FontMetrics *metrics,
                         int32_t &filled_x) const
{
    (void)metrics;
    ...
    if (*it != c)
        return drawCharDummy(gfx, x, y, fontWidth, fontHeight, style, filled_x);
...
size_t RLEfont::drawChar(LGFXBase *gfx, int32_t x, int32_t y, uint16_t code, const TextStyle *style,
                         FontMetrics *metrics, int32_t &filled_x) const
{ // RLE font
    (void)metrics;
    if ((code -= 0x20u) >= 0x60u)
...
GFXglyph *GFXfont::getGlyph(uint16_t uniCode) const
{
    ...
    if (uniCode > pgm_read_word(&last) || uniCode < f)
        return nullptr;
    ...
    size_t i = 0;
    while ((uniCode > pgm_read_word(&range_pst[i].end)) || (uniCode < pgm_read_word(&range_pst[i].start)))
    {
        if (++i == custom_range_num)
            return nullptr;
    }
...
size_t GFXfont::drawChar(LGFXBase *gfx, int32_t x, int32_t y, uint16_t uniCode, const TextStyle *style,
                         FontMetrics *metrics, int32_t &filled_x) const
{
    int32_t sy = 65536 * style->size_y;
    ...
    if (!glyph_)
    {
        ...
        if (glyph_)
            return drawCharDummy(gfx, x, y, pgm_read_byte(&glyph_->xAdvance), metrics->height, style, filled_x);
        return 0;
    }
    ...
    if (h)
    {
        ...
        int32_t btmp =
            pgm_read_byte(bitmap_); /// btmpの最上位ビット (符号ビット) をフラグとして扱うため敢えて uintにしない ;
        if (btmp & mask)
        {
            btmp = ~btmp;
        }
...
const uint8_t *U8g2font::getGlyph(uint16_t encoding) const
{
    ...
    if (encoding <= 255)
    {
        if (encoding >= 'a')
        {
            font += this->start_pos_lower_a();
        }
        else if (encoding >= 'A')
        {
            font += this->start_pos_upper_A();
        }

        for (; pgm_read_byte(&font[1]); font += pgm_read_byte(&font[1]))
        {
            if (pgm_read_byte(&font[0]) == encoding)
            {
                return font + 2;
...
size_t U8g2font::drawChar(LGFXBase *gfx, int32_t x, int32_t y, uint16_t uniCode, const TextStyle *style,
                          FontMetrics *metrics, int32_t &filled_x) const
{
    int32_t sy = 65536 * style->size_y;
    y += (metrics->y_offset * sy) >> 16;
    u8g2_font_decode_t decode(getGlyph(uniCode));
    if (decode.decode_ptr == nullptr)
        return drawCharDummy(gfx, x, y, this->max_char_width(), metrics->height, style, filled_x);
...
bool VLWfont::loadFont(DataWrapper *data)
{
    ...
    if (nullptr == gBitmap)
        gBitmap = (uint32_t *)heap_alloc(gCount * 4); // seek pointer to glyph bitmap in the file
    if (nullptr == gUnicode)
        gUnicode = (uint16_t *)heap_alloc(gCount * 2); // Unicode 16 bit Basic Multilingual Plane (0-FFFF)
    if (nullptr == gWidth)
        gWidth = (uint8_t *)heap_alloc(gCount); // Width of glyph
    if (nullptr == gxAdvance)
        gxAdvance = (uint8_t *)heap_alloc(gCount); // xAdvance - to move x cursor
    if (nullptr == gdX)
        gdX = (int8_t *)heap_alloc(gCount); // offset for bitmap left edge relative to cursor X

    if (!gUnicode || !gBitmap || !gWidth || !gxAdvance || !gdX)
    {
    ...
    do
    {
        ...
        if (gWidth)
            gWidth[gNum] = w;
        if (gxAdvance)
            gxAdvance[gNum] = (uint8_t)getSwap32(buffer[3]); // xAdvance - to move x cursor
        if (gdX)
            gdX[gNum] = (int8_t)getSwap32(buffer[5]); // x delta from cursor
...
size_t VLWfont::drawChar(LGFXBase *gfx, int32_t x, int32_t y, uint16_t code, const TextStyle *style,
                         FontMetrics *metrics, int32_t &filled_x) const
{
...
const IFont *fontdata[] = {
    ...
    &fonts::Font6, &fonts::Font7, &fonts::Font8, nullptr,
};
