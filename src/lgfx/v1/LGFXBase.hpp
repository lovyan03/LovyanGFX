...
//----------------------------------------------------------------------------
  ...
  public:
    ...
    void panel(IPanel* panel);

    void init(void)               { init_impl(true ); };
    void begin(void)              { init_impl(true ); };
    void init_without_reset(void) { init_impl(false); };
...
#define LGFX_INLINE_T template<typename T> __attribute__ ((always_inline)) inline
    ...
    LGFX_INLINE IPanel* panel(void) const { return _panel; }
    ...
    LGFX_INLINE_T void fillScreen  ( const T& color) { setColor(color); fillRect(0, 0, _width, _height); }
    LGFX_INLINE   void fillScreen  ( void )          {                  fillRect(0, 0, _width, _height); }
    ...
    LGFX_INLINE   std::int32_t width (void) const { return _width; }
    LGFX_INLINE   std::int32_t height(void) const { return _height; }
    ...
    LGFX_INLINE   void writeCommand(  std::uint8_t  cmd) { _panel->writeCommand(                  cmd , 1); } // AdafruitGFX compatible
    LGFX_INLINE   void writecommand(  std::uint8_t  cmd) { _panel->writeCommand(                  cmd , 1); } // TFT_eSPI compatible
    LGFX_INLINE   void writeCommand16(std::uint16_t cmd) { _panel->writeCommand(__builtin_bswap16(cmd), 2); }

    LGFX_INLINE   void spiWrite(   std::uint8_t  data) { _panel->writeData(                  data , 1); } // AdafruitGFX compatible
    LGFX_INLINE   void writedata(  std::uint8_t  data) { _panel->writeData(                  data , 1); } // TFT_eSPI compatible
    LGFX_INLINE   void writeData(  std::uint8_t  data) { _panel->writeData(                  data , 1); }
    LGFX_INLINE   void writeData16(std::uint16_t data) { _panel->writeData(__builtin_bswap16(data), 2); }
    LGFX_INLINE   void writeData32(std::uint32_t data) { _panel->writeData(__builtin_bswap32(data), 4); }
    LGFX_INLINE   std::uint8_t  readData8( std::uint8_t index=0) { return                   _panel->readData(index, 1) ; }
    LGFX_INLINE   std::uint16_t readData16(std::uint8_t index=0) { return __builtin_bswap16(_panel->readData(index, 2)); }
    LGFX_INLINE   std::uint32_t readData32(std::uint8_t index=0) { return __builtin_bswap32(_panel->readData(index, 4)); }

    //LGFX_INLINE   void writeBytes(const std::uint8_t* data, std::int32_t length, bool use_dma = false) { _panel->writeBytes(data, length, use_dma); }
    //LGFX_INLINE   void readBytes(std::uint8_t* dst, std::int32_t length) { _panel->readBytes(dst, length); }
    LGFX_INLINE   std::uint32_t readPanelID(void) { return _panel->readPanelID(); }
...
//----------------------------------------------------------------------------
    ...
    {
      RGBColor data[1];
      if (x < 0 || x >= _width || y < 0 || y >= _height) return data[0];
...
//----------------------------------------------------------------------------
  ...
  protected:
    ...
    virtual void init_impl(bool use_reset);
...
//----------------------------------------------------------------------------
  ...
  public:
    ...
    void sleep(void) { _panel->setSleep(true); }
    void wakeup(void) { _panel->setSleep(false); }
    void partialOn(void) { _panel->setIdol(true); }
    void partialOff(void) { _panel->setIdol(false); }
  ...
  protected:
    ...
    void calibrate_touch(std::uint16_t *parameters, std::uint32_t fg_rawcolor, std::uint32_t bg_rawcolor, std::uint8_t size)
    {
      ...
      _touch->setCalibrate(orig, _width, _height);
