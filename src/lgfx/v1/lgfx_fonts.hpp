...
struct IFont
{
    ...
    virtual font_type_t getType(void) const
    {
        return font_type_t::ft_unknown;
    }
    ...
    virtual bool unloadFont(void)
    {
        return false;
    }
    virtual size_t drawChar(LGFXBase *gfx, int32_t x, int32_t y, uint16_t c, const TextStyle *style,
                            FontMetrics *metrics, int32_t &filled_x) const = 0;

  protected:
    size_t drawCharDummy(LGFXBase *gfx, int32_t x, int32_t y, int32_t w, int32_t h, const TextStyle *style,
...
struct BaseFont : public IFont
{
    ...
    constexpr BaseFont() : void_chartbl{nullptr}, widthtbl{nullptr}, width{0}, height{0}, baseline{0}
    {
    }
    constexpr BaseFont(const void *char_tbl, const uint8_t *width_tbl, uint8_t w, uint8_t h, uint8_t bl)
        : void_chartbl{char_tbl}, widthtbl{width_tbl}, width{w}, height{h}, baseline{bl}
    {
    }
...
{
    constexpr GLCDfont(const void *char_tbl, const uint8_t *width_tbl, uint8_t w, uint8_t h, uint8_t bl)
        : BaseFont(char_tbl, width_tbl, w, h, bl)
    {
    }
    font_type_t getType(void) const override
    {
        return ft_glcd;
    }

    bool updateFontMetric(FontMetrics *metrics, uint16_t uniCode) const override;
    size_t drawChar(LGFXBase *gfx, int32_t x, int32_t y, uint16_t c, const TextStyle *style, FontMetrics *metrics,
                    int32_t &filled_x) const override;
};

struct FixedBMPfont : public BaseFont
{
    constexpr FixedBMPfont(const void *char_tbl, const uint8_t *width_tbl, uint8_t w, uint8_t h, uint8_t bl)
        : BaseFont(char_tbl, width_tbl, w, h, bl)
    {
    }
    font_type_t getType(void) const override
...
{
    constexpr BMPfont(const void *char_tbl, const uint8_t *width_tbl, uint8_t w, uint8_t h, uint8_t bl)
        : BaseFont(char_tbl, width_tbl, w, h, bl)
    {
    }
    font_type_t getType(void) const override
...
{
    constexpr RLEfont(const void *char_tbl, const uint8_t *width_tbl, uint8_t w, uint8_t h, uint8_t bl)
        : BMPfont(char_tbl, width_tbl, w, h, bl)
    {
    }
    font_type_t getType(void) const override
    {
...
struct BDFfont : public BaseFont
{
    ...
    uint8_t y_advance;
    BDFfont() = default;
    constexpr BDFfont(const void *char_tbl, const uint16_t *index_tbl, uint16_t index_size, uint8_t w,
                      uint8_t half_width, uint8_t h, uint8_t bl, uint8_t y_adv)
        : BaseFont{char_tbl, nullptr, w, h, bl}, indextbl{index_tbl}, indexsize{index_size}, halfwidth{half_width},
          y_advance{y_adv}
    {
    }
    font_type_t getType(void) const override
...
{                         // Data stored for FONT AS A WHOLE:
    ...
    constexpr GFXfont(uint8_t *bitmap_, GFXglyph *glyph_, uint16_t first_, uint16_t last_, uint8_t yAdvance_,
                      uint16_t range_num_ = 0, EncodeRange *range_ = nullptr)
        : bitmap(bitmap_), glyph(glyph_), first(first_), last(last_), yAdvance(yAdvance_), range_num(range_num_),
          range(range_)
    {
    }
...
struct U8g2font : public lgfx::IFont
{
    ...
    uint8_t glyph_cnt(void) const
    {
        return pgm_read_byte(&_font[0]);
    }
    uint8_t bbx_mode(void) const
    {
        return pgm_read_byte(&_font[1]);
    }
    uint8_t bits_per_0(void) const
    {
        return pgm_read_byte(&_font[2]);
    }
    uint8_t bits_per_1(void) const
    {
        return pgm_read_byte(&_font[3]);
    }
    uint8_t bits_per_char_width(void) const
    {
        return pgm_read_byte(&_font[4]);
    }
    uint8_t bits_per_char_height(void) const
    {
        return pgm_read_byte(&_font[5]);
    }
    uint8_t bits_per_char_x(void) const
    {
        return pgm_read_byte(&_font[6]);
    }
    uint8_t bits_per_char_y(void) const
    {
        return pgm_read_byte(&_font[7]);
    }
    uint8_t bits_per_delta_x(void) const
    {
        return pgm_read_byte(&_font[8]);
    }
    int8_t max_char_width(void) const
    {
        return pgm_read_byte(&_font[9]);
    }
    int8_t max_char_height(void) const
    {
        return pgm_read_byte(&_font[10]);
    } /* overall height, NOT ascent. Instead ascent = max_char_height + y_offset */
    int8_t x_offset(void) const
    {
        return pgm_read_byte(&_font[11]);
    }
    int8_t y_offset(void) const
    {
        return pgm_read_byte(&_font[12]);
    }
    int8_t ascent_A(void) const
    {
        return pgm_read_byte(&_font[13]);
    }
    int8_t descent_g(void) const
    {
        return pgm_read_byte(&_font[14]);
    } /* usually a negative value */
    int8_t ascent_para(void) const
    {
        return pgm_read_byte(&_font[15]);
    }
    int8_t descent_para(void) const
    {
        return pgm_read_byte(&_font[16]);
    }

    uint16_t start_pos_upper_A(void) const
    {
        return pgm_read_byte(&_font[17]) << 8 | pgm_read_byte(&_font[18]);
    }
    uint16_t start_pos_lower_a(void) const
    {
        return pgm_read_byte(&_font[19]) << 8 | pgm_read_byte(&_font[20]);
    }
    uint16_t start_pos_unicode(void) const
    {
        return pgm_read_byte(&_font[21]) << 8 | pgm_read_byte(&_font[22]);
    }
...
static constexpr lgfx::BDFfont efont = {
    (const uint8_t *)efontFontData, efontFontList, sizeof(efontFontList) >> 1, 16, 8, 16, 14, 16};
#endif
