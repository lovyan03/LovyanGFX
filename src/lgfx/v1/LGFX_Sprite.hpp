...
//----------------------------------------------------------------------------
  ...
  protected:
    ...
    std::uint32_t _index;
    std::uint_fast16_t _xptr;
    std::uint_fast16_t _yptr;
    std::uint_fast16_t _bitwidth;

    inline std::uint32_t ptr_advance(std::uint32_t length = 1)
    {
      if ((_xptr += length) <= _xe) return (_index += length);
      _xptr = _xs;
      if (++_yptr > _ye)
      {
        _yptr = _ys;
...
//----------------------------------------------------------------------------
  ...
  protected:
    ...
    void push_sprite(LovyanGFX* dst, std::int32_t x, std::int32_t y, std::uint32_t transp = ~0u)
    {
      ...
      dst->pushImage(x, y, _width, _height, &p, _panel_sprite.getSpriteBuffer()->use_dma()); // DMA disable with use SPIRAM
    }
