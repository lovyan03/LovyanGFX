...
static HuffTable gHuffTab[4];
static uint8* gHuffVal[4] = { gHuffVal0, gHuffVal1, gHuffVal2, gHuffVal3 };
...
static uint8 readDHTMarker(void)
{
   ...
   while (left)
   {
      ...
      pHuffTable = &gHuffTab[tableIndex];
      pHuffVal = gHuffVal[tableIndex];
...
static uint8 readSOSMarker(void)
{
   ...
   for (i = 0; i < gCompsInScan; i++)
   {
      ...
      gCompList[i]    = ci;
      gCompDCTab[ci] = ((c >> 4) & 15) ? 1 : 0;
      gCompACTab[ci] = (c & 15) ? 1 : 0;
   }
...
// 1/cos(4*pi/16)
// 362, 256+106
__attribute__ ((always_inline)) inline static PJPG_INLINE int16 imul_b1_b3(int16 w)
{
   return ((w * 362L) + 128L) >> 8;
...
// 1/cos(6*pi/16)
// 669, 256+256+157
__attribute__ ((always_inline)) inline static PJPG_INLINE int16 imul_b2(int16 w)
{
   return ((w * 669L) + 128L) >> 8;
...
// 1/cos(2*pi/16)
// 277, 256+21
__attribute__ ((always_inline)) inline static PJPG_INLINE int16 imul_b4(int16 w)
{
  return ((w * 277L) + 128L) >> 8;
...
// 196, 196
__attribute__ ((always_inline)) inline static PJPG_INLINE int16 imul_b5(int16 w)
{
   return ((w * 196L) + 128L) >> 8;
...
static uint8 decodeNextMCU(void)
{
   ...
   for (mcuBlock = 0; mcuBlock < gMaxBlocksPerMCU; mcuBlock++)
   {
      ...
      uint8 s = huffDecode(&gHuffTab[compDCTab], gHuffVal[compDCTab]);
      ...
      HuffTable* huffTab = &gHuffTab[compACTab ? 3 : 2];
      uint8* huffVal =  gHuffVal[compACTab ? 3 : 2];
