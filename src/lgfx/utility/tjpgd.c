...
)
{
	...
	do {	/* Process all tables in the segment */
//		if (( ndata -= 17 ) < 0) return JDR_FMT1;	/* Err: wrong data size */
		...
		do {
			...
			do {
				do {
					...
					*rgb24++ = BYTECLIP(yy - (((int32_t)(0.34414 * (1<<FP_SHIFT)) * cb + (int32_t)(0.71414 * (1<<FP_SHIFT)) * cr) >> FP_SHIFT));
