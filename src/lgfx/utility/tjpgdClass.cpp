...
)
{
  ...
  for (y = 0; y < height; y += my) {    /* Vertical loop of MCUs */
    for (x = 0; x < width; x += mx) {  /* Horizontal loop of MCUs */
      ...
      rc = mcu_load(this, mcubuf, (int32_t*)workbuf[workflip]);    /* Load an MCU (decompress huffman coded stream and apply IDCT) */
      if (rc != JDR_OK) return rc;
      rc = mcu_output(this, mcubuf, (uint8_t*)workbuf[workflip], outfunc, x, y);  /* Output the MCU (color space conversion, scaling and output) */
      if (rc != JDR_OK) return rc;
      workflip = !workflip;
    }
